...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;font-family:'Poppins',sans-serif}
    body{margin:0;background:linear-gradient(180deg,var(--bg),#ebf6ff);min-height:100vh;display:flex;align-items:flex-start;justify-content:center;padding:36px}
    .wrap{width:980px;max-width:95vw}
    ...
    header h1{font-size:1.35rem;color:#0f172a;margin:0}
    .muted{color:var(--muted);font-size:0.9rem}
    .grid{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:18px}
    .card{background:var(--card);border-radius:12px;padding:14px 14px 18px;box-shadow:0 8px 24px rgba(15,23,42,0.06);border:1px solid rgba(15,23,42,0.03)}
    .card .title{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}
    .card .title .label{font-weight:700;color:#04447b;display:flex;align-items:center;gap:8px}
    .card .title i{color:#2563eb}
    .controls{display:flex;gap:8px;align-items:center}
    .controls button{background:var(--accent);border:none;color:white;padding:8px 12px;border-radius:10px;cursor:pointer;font-weight:600}
    .controls button.ghost{background:#eef6ff;color:var(--accent);border:1px solid rgba(37,99,235,0.12)}
    table{width:100%;border-collapse:collapse;background:transparent}
    th, td{padding:8px 6px;border:1px solid var(--table-border);text-align:center;font-size:0.95rem}
    th{background:#f8fbff;font-weight:700;color:#0b3a66}
    td input{width:100%;border:0;padding:6px;text-align:center;font-weight:600;background:transparent}
    td input:focus{outline:none;background:#eef6ff}
    .summary{display:flex;align-items:center;gap:12px;margin-top:8px}
    .sum-box{background:linear-gradient(180deg,#fff,#f6fbff);padding:12px 16px;border-radius:10px;min-width:120px;text-align:center;box-shadow:inset 0 -6px 12px rgba(37,99,235,0.03)}
    .sum-box.small{min-width:88px}
    .sum-label{font-size:0.85rem;color:var(--muted)}
    .sum-value{font-size:1.25rem;font-weight:800;color:#0b2540;margin-top:6px}
    .total-big{flex:1;background:linear-gradient(90deg,#fef3c7,#fdba74);padding:10px;border-radius:10px;font-weight:800;color:#92400e;text-align:left}
    .total-big span{float:right;font-size:1.1rem}
    .bottom-actions{display:flex;gap:8px;justify-content:flex-end;margin-top:10px}
    .small-btn{background:#fff;border:1px solid #e6eef9;padding:8px 10px;border-radius:8px;cursor:pointer}
    @media(max-width:880px){.grid{grid-template-columns:1fr}.total-big{font-size:0.95rem}}
  </style>
</head>
<body>
  <div class="wrap">
...
<script>
  ...
  function importSample(){ location.reload(); }
</script>
</body>
</html>
